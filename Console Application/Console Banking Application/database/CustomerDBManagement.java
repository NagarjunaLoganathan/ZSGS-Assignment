...
public class CustomerDBManagement extends JDBC_Connection {
	...
	private static final String UPI_CHECK = "SELECT UPI_ID FROM customer_details WHERE UPI_ID = ?";
	...
	public Boolean accCheck(short accNo) {
		...
		try {
			...
			System.out.println("Unable to identify this Account.");
		}
		if (count > 0) {
			closer();
			return true;
	...
	public Boolean upiCheck(String upiId) {
		...
		try {
			...
			System.err.println("Unable to identify this UPI ID.");
		}
		if (count > 0) {
			closer();
			return true;
